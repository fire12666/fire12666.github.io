...
exports.generateBCMainG = generateBCMainG;
exports.appendBCCenter = appendBCCenter;
exports.appendBCAxes = appendBCAxes;
exports.appendBCTeamSeries = appendBCTeamSeries;
exports.generateSPG1 = generateSPG1;
exports.generateSPG2 = generateSPG2;
exports.generateSPG3 = generateSPG3;
exports.appendSPAxes1 = appendSPAxes1;
exports.appendSPAxes2 = appendSPAxes2;
exports.appendSPAxes3 = appendSPAxes3;
exports.appendSPGraphLabels = appendSPGraphLabels;
exports.drawSPXAxis = drawSPXAxis;
exports.drawSPYAxis = drawSPYAxis;
exports.placeSPTitle = placeSPTitle;
...
function appendBCTeamSeries(chartData, juveIdx) {
  d3.select("#bc-center").selectAll(".series").data(chartData).enter().append("g").attr("class", function (d, i) {
    return i == juveIdx ? "series juve" : "series";
  }).transition().duration(2000).attr("opacity", 1).attr("fill", function (d, i) {
...
function appendSPGraphLabels(g, width) {
  ...
  g.append('text').text('Numéro de match').attr('class', 'x axis-text').attr('font-size', 12);
}
/**
...
function drawSPXAxis(axis, xScale, height) {
  axis.attr('transform', 'translate( 0, ' + height + ')').call(d3.axisBottom(xScale).tickSizeOuter(0).tickArguments([20, '~s']));
}
/**
...
function drawSPYAxis(axis, yScale, width) {
  axis.attr('transform', 'translate(' + width + ', 0)').call(d3.axisLeft(yScale).tickSizeOuter(0).tickArguments([8, '~s']));
}
/**
...
exports.preprocessOffense = preprocessOffense;
exports.preprocessDefense = preprocessDefense;
exports.preprocessTooltipOff = preprocessTooltipOff;
exports.preprocessTooltipDef = preprocessTooltipDef;
exports.getLeftTeamNames = getLeftTeamNames;
exports.getRightTeamNames = getRightTeamNames;
exports.getBumpChartData = getBumpChartData;
...
function getLeftTeamNames(data) {
  var leftTeamNames = [];
  data['2019-2020'].forEach(function (ranking) {
...
function getRightTeamNames(data) {
  var rightTeamNames = [];
  data['2021-2022'].forEach(function (ranking) {
...
function getBumpChartData(data) {
  var teams = getLeftTeamNames(data).sort();
  var seasons = Object.keys(data);
  var teamsIdx = new Map(teams.map(function (team, i) {
    return [team, i];
  }));
  var seasonsIdx = new Map(seasons.map(function (season, i) {
    return [season, i];
  })); // Initialize the ranking matrix

  var matrix = Array.from(teamsIdx, function () {
    return new Array(seasons.length).fill(null);
  });

  var _iterator = _createForOfIteratorHelper(Object.entries(data).reverse()),
      _step;

  try {
    var _loop = function _loop() {
      var _step$value = _slicedToArray(_step.value, 2),
          season = _step$value[0],
          rankings = _step$value[1];

      rankings.forEach(function (ranking, idx) {
        var seasonIdx = seasonsIdx.get(season);
        var teamIdx = teamsIdx.get(ranking.Team);
        var next = null;
...
},{}],"scripts/radar_chart.js":[function(require,module,exports) {
"use strict";
...
exports.drawSteps = drawSteps;
exports.drawPoints = drawPoints;
exports.drawAxes = drawAxes;
exports.drawAxesLabel = drawAxesLabel;
...
var TOTAL_HEADERS_COUNT = 2;
var POINT_SMALL_RADIUS = 3;
var POINT_LARGE_RADIUS = 5;
var N_STEPS = 4;
var TITLE_VERTICAL_MARGIN = 35;
var TITLE_FONT_SIZE = 17;
var STEP_OPACITY = 0.5;
var STEP_FONT_SIZE = 10;
var LABELS_FONT_SIZE = 13;
var SHAPE_SMALL_STROKE_WIDTH = 2;
var SHAPE_LARGE_STROKE_WIDTH = 3;
var AXE_STROKE_WIDTH = .5;
var TICK_STROKE_WIDTH = .3;
var LABELS_MULTIPLIER_FACTOR = 1.05;
var BACKGROUND_COLOR = 'white'; // Constants used to move scales.

var MAX_MULTPLIER_FACTOR = 1.2;
var MIN_MULTIPLIER_FACTOR = 0.9;
var XGA_MULTIPLIER_FACTOR = 1.1; // Possible steps used for viz.

var STEP_CHOICES = [.1, .2, .5, 1, 2, 5, 10, 20, 50, 100];
/**
...
function setScale(stepChoice, minValue) {
  ...
  return d3.scaleLinear().domain([minValue, maxValue]).range([0, 1]);
}
/**
...
function setSteps(data, attribute) {
  ...
  for (var i in _toConsumableArray(Array(STEP_CHOICES.length).keys())) {
...
function drawSteps(stepChoices, mins, scales, element, xCenter, yCenter) {
  var nCircles = _toConsumableArray(Array(N_STEPS - 1).keys()).map(function (i) {
    return i + 1;
  });

  var labels = Object.keys(mins);
  var totalAxes = labels.length;
  var stepContainer = element.append('g'); // Adds text for each step of each category.
...
function drawAxes(data, element, xCenter, yCenter) {
  ...
  var axesData = _toConsumableArray(Array(totalAxes).keys()); // Generates an array from 0 to the number of axes.
...
function drawAxesLabel(data, element, xCenter, yCenter, tip) {
  var stats = getStats(data); // Build array containing labels.
  ...
  container.selectAll('text').data(data).enter().append('text').attr('class', 'label').attr('x', function (_, i) {
    ...
    tip.show(d, this);
    d3.select(this).style('font-size', 16 + 'px');
  }).on("mouseout", function () {
    tip.hide();
...
function drawTicks(stepChoices, mins, scales, element, xCenter, yCenter) {
  var nTicks = _toConsumableArray(Array(N_STEPS - 1).keys()).map(function (i) {
    return i + 1;
  });
...
exports.drawOffensiveRadarCharts = drawOffensiveRadarCharts;
exports.drawDefensiveRadarChart = drawDefensiveRadarChart;
exports.addButtons = addButtons;
exports.positionSPLabels = positionSPLabels;
exports.drawSPCircles = drawSPCircles;
exports.drawSPDensityLine = drawSPDensityLine;
exports.setSPTitleText = setSPTitleText;
exports.updateBCXScales = updateBCXScales;
exports.updateBCYScales = updateBCYScales;
exports.drawBCXAxis = drawBCXAxis;
exports.drawBCYAxis = drawBCYAxis;
exports.drawBCDashedLines = drawBCDashedLines;
exports.drawBCRankingLines = drawBCRankingLines;
exports.drawBCBumpNumber = drawBCBumpNumber;

var radarChart = _interopRequireWildcard(require("./radar_chart"));
...
/*********************************** CONSTANTS USED FOR STYLE **********************************/
var JUVENTUS_COLOR = 'blue';
var TOP_7_COLOR = 'orange';
var BUTTON_JUVENTUS_WIDTH = 125;
var BUTTON_TOP_7_WIDTH = 100;
var BUTTON_HEIGHT = 25;
var BUTTON_BACKGROUND_COLOR = 'grey';
var LOW_OPACITY = 0.15;
/************************************** V1/2 - RADAR CHART *************************************/
...
function drawOffensiveRadarCharts(data, element, width, height, margin, tip) {
  var xCenter = width / 2;
  var yCenter = height / 2; // Erases charts so they can be rebuilt.

  radarChart.eraseAll(element); // Prepares scales, steps and mins.
  ...
  var container = element.append('g').attr('transform', 'translate(' + margin.left + ', ' + margin.top + ')'); // Build each chart separately.
...
function addButtons(svgElement, graphHeight, margin) {
  var toggled = false; // Create button for the Juventus radar chart
  ...
  }).style('cursor', 'pointer'); // Create button for the Top 7 radar chart
...
function drawSPCircles(chart, data, radius, colorScale, xScale, yScale) {
  ...
  if (chart.selectAll('circle').empty()) {
    chart.selectAll('dot').data(data).enter().append('circle').attr('cx', function (d) {
...
function drawSPDensityLine(chart, data, colorScale, result, xScale, yScale) {
  ...
  var pas = Math.round((salaireMax - salaireMin) / 15); // point extrême inférieur

  points.push([xScale(0), yScale(salaireMin - pas / 2)]); // les points obtenus de nos données
...
function drawBCDashedLines(seasons, bxScale, height, padding) {
  d3.select("#bc-center").selectAll(".dash-line").data((0, _util.range)(0, seasons.length)).enter().append("path").attr('class', 'dash-line').attr("stroke", "#ccc").attr("stroke-width", 2).attr("stroke-dasharray", "5,5").attr("d", function (d) {
    return d3.line()([[bxScale(d), padding / 2], [bxScale(d), height - padding / 2]]);
  });
}
/**
 * Draws dashed lines on top of the seasons
...
function drawBCRankingLines(bxScale, byScale) {
  d3.selectAll(".series").selectAll(".ranking-line").data(function (d) {
    return d;
  }).enter().append("path").attr('class', 'ranking-line').attr("stroke-width", "5").attr("d", function (d, i) {
    if (d.next) return d3.line()([[bxScale(i), byScale(d.rank)], [bxScale(i + 1), byScale(d.next.rank)]]);
  });
}
/**
 * Draws dashed lines on top of the seasons
...
function drawBCBumpNumber(bxScale, byScale, bumpRadius) {
  var bumps = d3.selectAll(".series").selectAll("g").data(function (d) {
    return d;
  }).enter().append("g").attr("class", "bump").attr("transform", function (d, i) {
    return "translate(".concat(bxScale(i), ",").concat(byScale(d.rank), ")");
  }).call(function (g) {
    return g.append("title").text(function (d, i) {
      return "".concat(d.team, " - ").concat(d.season, "\nPoints: ").concat(d.pts == 0 ? "check for updates" : d.pts);
    });
  });
...
},{"./radar_chart":"scripts/radar_chart.js","./preprocess.js":"scripts/preprocess.js","./util":"scripts/util.js"}],"scripts/scales.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.setSPColorScale = setSPColorScale;
exports.setSPXScale = setSPXScale;
exports.setSPYScale = setSPYScale;
...
function setSPYScale(height, data) {
  return d3.scaleLinear().domain([d3.min(data, function (d) {
...
exports.drawRCLegend = drawRCLegend;
exports.drawSPLegend = drawSPLegend;
...
function drawRCLegend(colorScale, g, height, margin) {
  var legend = _d3SvgLegend.default.legendColor().labelFormat(d3.format('.2f')).title('Légende').shape('circle').shapeRadius(10).scale(colorScale);
...
function drawSPLegend(g, colorScale, positionx, positiony) {
  ...
  var legendOrdinal = _d3SvgLegend.default.legendColor().shape('path', d3.symbol().type(d3.symbolCircle).size(80)()) // use cellFilter to hide the "e" cell
  .cellFilter(function (d) {
    return d.label !== 'e';
  }).scale(colorScale);

  g.select('.legendOrdinal').call(legendOrdinal);
  g.select('.legendOrdinal').select('text.legend-title').attr('transform', 'translate(0, -20)').text('Légende');
}
},{"d3-svg-legend":"../node_modules/d3-svg-legend/indexRollupNext.js"}],"scripts/hover.js":[function(require,module,exports) {
"use strict";
...
(function (d3) {
  var svgSize;
  var graphSize;
  var MIN_RC_HEIGHT = 550;
  var MIN_RC_WIDTH = 1000;
  var MAX_RC_HEIGHT = 650;
  var MAX_RC_WIDTH = 1500;
  var MIN_SP_HEIGHT = 400;
  var MIN_SP_WIDTH = 481;
  /**
  ...
  function setSizing(dim, selection, margin, isRadarChart) {
    ...
    var width = isRadarChart ? svgSize.width / 3 : svgSize.width;
    var height = isRadarChart ? svgSize.height - 100 : svgSize.height;
    graphSize = {
  ...
  var tip = (0, _d3Tip.default)().attr('class', 'd3-tip').html(function (d) {
    return hover.getContents(d);
  });
  d3.select('.chart-off-svg').call(tip); // first viz
  ...
  d3.csv('./data_offensive.csv', d3.autoType).then(function (data) {
    ...
    var bounds = d3.select('#chart-off').node().getBoundingClientRect();
    var selection = d3.select(".chart-off-svg");
  ...
  d3.csv('./data_defensive.csv', d3.autoType).then(function (data) {
    ...
    var bounds = d3.select('#chart-def').node().getBoundingClientRect();
    var selection = d3.select(".chart-def-svg");
    var vizWidth = bounds.width > MAX_RC_WIDTH ? MAX_RC_WIDTH : bounds.width;
    vizWidth = bounds.width < MIN_RC_WIDTH ? MIN_RC_WIDTH : vizWidth;
    var vizHeight = bounds.height > MAX_RC_HEIGHT ? MAX_RC_HEIGHT : bounds.height;
    vizHeight = bounds.height < MIN_RC_HEIGHT ? MIN_RC_HEIGHT : vizHeight;
  ...
  d3.json('./serieA_ranking.json', d3.autoType).then(function (data) {
    ...
    var bxScale = d3.scalePoint();
    var yScale = d3.scalePoint();
    var byScale = d3.scalePoint();
    var chartData = preproc.getBumpChartData(data);
    var leftTeamNames = preproc.getLeftTeamNames(data);
    var rightTeamNames = preproc.getRightTeamNames(data);
    var teamNames = preproc.getLeftTeamNames(data).sort();
    var xDomain = Object.keys(data);
    var bounds = d3.select('#chart-rank').node().getBoundingClientRect();
    var selection = d3.select(".chart-rank-svg");
    var mainG = helper.generateBCMainG(marginBC);
    var centerG = helper.appendBCCenter(mainG);
    helper.appendBCAxes(mainG);
    var width = xDomain.length * 60 + marginBC.left + marginBC.right + padding * 2;
    var height = leftTeamNames.length * 60 + marginBC.top + marginBC.bottom + padding * 2;
    setSizing({
    ...
    function buildBumpChart() {
      var xRangeInterval = [padding, graphSize.width - padding];
      var yRangeInterval = [padding, graphSize.height - padding];
    ...
    window.addEventListener('resize', function () {
      var bounds = d3.select('#chart-rank').node().getBoundingClientRect();
      var selection = d3.select(".chart-rank-svg");
      var width = xDomain.length * 100 + marginBC.left + marginBC.right + padding * 2;
      var height = leftTeamNames.length * 60 + marginBC.top + marginBC.bottom + padding * 2;
      setSizing({
        width: Math.max(width, bounds.width * 0.8),
        height: Math.max(height, bounds.height)
  ...
  var radius = 4;
  d3.csv('./totalSalaries2019.csv').then(function (data) {
    var g1 = helper.generateSPG1(marginSP);
    var bounds = d3.select('#chart-eff').node().getBoundingClientRect();
    var selection = d3.select("#chart-eff-svg1");
    setSizing({
      width: MIN_SP_WIDTH,
    ...
    viz.setSPTitleText();
    var chart1 = d3.select('#chart-eff-svg1').select('#graph-g1');
    var axisx1 = d3.select('.x1.axis');
    var axisy1 = d3.select('.y1.axis');
    var colorScale = scales.setSPColorScale();
    var xScale = scales.setSPXScale(graphSize.width, data);
    var yScale = scales.setSPYScale(graphSize.height, data);
  ...
  d3.csv('./totalSalaries2020.csv').then(function (data) {
    var g2 = helper.generateSPG2(marginSP);
    var selection = d3.select("#chart-eff-svg2");
    setSizing({
      width: MIN_SP_WIDTH,
      height: MIN_SP_HEIGHT
    }, selection, marginSP, false);
    ...
    viz.setSPTitleText();
    var chart2 = d3.select('#chart-eff-svg2').select('#graph-g2');
    var axisx2 = d3.select('.x2.axis');
    var axisy2 = d3.select('.y2.axis');
    var colorScale = scales.setSPColorScale();
    var xScale = scales.setSPXScale(graphSize.width, data);
    var yScale = scales.setSPYScale(graphSize.height, data);
  ...
  d3.csv('./totalSalaries2021.csv').then(function (data) {
    var g3 = helper.generateSPG3(marginSP);
    var selection = d3.select("#chart-eff-svg3");
    setSizing({
      width: MIN_SP_WIDTH,
      height: MIN_SP_HEIGHT
    }, selection, marginSP, false);
    ...
    viz.setSPTitleText();
    var chart3 = d3.select('#chart-eff-svg3').select('#graph-g3');
    var axisx3 = d3.select('.x3.axis');
    var axisy3 = d3.select('.y3.axis');
    var colorScale = scales.setSPColorScale();
    var xScale = scales.setSPXScale(graphSize.width, data);
    var yScale = scales.setSPYScale(graphSize.height, data);
...
},{"./scripts/helper.js":"scripts/helper.js","./scripts/preprocess.js":"scripts/preprocess.js","./scripts/viz.js":"scripts/viz.js","./scripts/scales.js":"scripts/scales.js","./scripts/legend.js":"scripts/legend.js","./scripts/hover.js":"scripts/hover.js","d3-tip":"../node_modules/d3-tip/index.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
